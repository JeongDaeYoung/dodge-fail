...
<!-- 
   ...
   font-size:15px;}
	...
	.data_tbl {margin:0 0 0 -1px;border-top:2px solid #005AAB; width:100%; border-collapse:collapse}
.data_tbl thead th {padding:6px 0;color:#333;background:#f8f8f8;border-bottom:1px solid #AAA;border-left:1px solid #dddee2;text-align:center;}
.data_tbl tbody th {padding:9px 10px 10px;background:#f8f8f8;text-align: right;border-bottom:1px solid #dddee2;}
.data_tbl td {padding:9px 7px 10px;border-left:1px solid #dddee2;border-bottom:1px solid #dddee2; text-align:center; font-size: 12px;letter-spacing:-1px;word-break:keep-all;}
.data_tbl .last td, .data_tbl .last th {border-bottom:2px solid #AAA;}
.data_tbl tbody tr:hover {background: #f6f6f6;}
.data_tbl td.th_code { font-size: 11px; font-family: tahoma;}
.data_tbl td.th_tit {color:#eee; font-weight: bold;}
.data_tbl .c-btn04, .data_tbl02 .c-btn04 {vertical-align: middle;}
.data_tbl .c-btn04 span, .data_tbl02 .c-btn04 span {vertical-align: top;}

.data_tbl02 {margin:0 0 0 -1px;border-top:2px solid #005AAB; width:100%; border-collapse:collapse}
.data_tbl02 tbody th {padding:12px 10px;background:#f8f8f8;text-align: right;border-bottom:1px solid #dddee2;}
.data_tbl02 td {padding:12px 10px;border-left:1px solid #dddee2;border-bottom:1px solid #dddee2; }
.data_tbl02 .last td, .data_tbl02 .last th {border-bottom:2px solid #AAA;}
.t_non {border-top:none}

.inner_tbl {width:100%; border-collapse:collapse}
.inner_tbl tbody th {padding:10px;background:#fafafa;text-align: center;border-bottom:1px solid #dddee2;}
.inner_tbl td {padding:10px;border-left:1px solid #dddee2;border-bottom:1px solid #dddee2; }
.data_tbl02 .inner_tbl td, .data_tbl02 .inner_tbl th {border-bottom:1px solid #dddee2;}-->
...
<body>
   ...
   <IMG SRC="${pageContext.request.contextPath}/resources/images/YSUicon.ico" WIDTH="120" ALIGN="MIDDLE">
   <FONT SIZE="8" FACE="Malgun Gothic">
   연성대학교 수강신청 장바구니</FONT>   
   <table border="2" width="300" height="100" align="right" class="stuinfo_tbl">
    <tr>
      <th>과</th>
      <th>학년</th>
      <th>반</th>
   ...
   <span style="float:right"><input name="logout" type="submit" value="로그아웃"> </span>
   </form>
   
   
   <div id="div11" class="padding2">
  		<!-- 시간표테이블  -->
      <center><b> 시 간 표 </b></center>
	...
	<div class="padding1">
		<center>장바구니</center>
		<table class="data_tbl" summary="코드, 과정명, 요일, 시간, 강의기간, 정원, 수강료, 강사명을 나타낸 표입니다.">
		<colgroup>
			<col  width="75px"><col width="*"><col width="*"><col width="*"><col width="*"><col width="*"><col width="*"><col width="*"><col width="*"><col width="*"><col width="*"><col width="*">
		</colgroup>
		<thead>
		<tr>
		...
		<tr>
		<td><input type="text" value="${item.kmId}" name="deletebasket" style="border:0; background-color:transparent; text-align: center; width:100%;" readonly/></td>
		<td>${item.kmName}</td>
		<td>${item.kmHak}</td>
		<td>${item.kmBan}</td>
		<td>${item.kmDay}</td>
		...
		<td>${item.kmIsu}</td>
		<td>${item.kmGrade}</td>
		<td><button type="submit" class="btn btn-sm btn-primary btn-block">장바구니취소</button></td>
		</tr>
		</form>
		</c:forEach>
	</tbody>
	...
	<div class="padding1" id="div1">
		...
		<table class="data_tbl" summary="코드, 과정명, 요일, 시간, 강의기간, 정원, 수강료, 강사명을 나타낸 표입니다.">
	<colgroup>
		<col  width="75px"><col width="*"><col width="*"><col width="*"><col width="*"><col width="*"><col width="*"><col width="*"><col width="*"><col width="*"><col width="*">
	</colgroup>
	<thead>
		<tr>
			<th rowspan="2" scope="col">코드</th>
	...
	<tbody>
		...
		<form action="subbasket" method="post">
		<tr>
		<td><input type="text" value="${item.kmId}" name="insertbasket" style="border:0; background-color:transparent; text-align: center; width:100%;" readonly/></td>
		<td>${item.kmName}</td>
		<td>${item.kmHak}</td>
		<td>${item.kmBan}</td>
		<td>${item.kmDay}</td>
		...
		<td>${item.kmIsu}</td>
		<td>${item.kmGrade}</td>
		<td><button type="submit" onclick="div1.reload" class="btn btn-sm btn-primary btn-block">장바구니담기</button></td>
		</tr>
		</form>
		</c:forEach>
	</tbody>
   ...
   <script type="text/javascript">
      (function() {
         ...
         var color = new Array("#dc3545", "#fd7e14", "#ffc107", "#28a745", "#007bff", "#20c997", "#17a2b8", "#e83e8c", "#6610f2"); // 배경색 배열
