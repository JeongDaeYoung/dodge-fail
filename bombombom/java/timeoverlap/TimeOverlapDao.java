...
public class TimeOverlapDao {
	...
	public List<TimeOverlap> timeList(String km_code, String stu_id) {
		List<TimeOverlap> results = jdbcTemplate.query(
				...
				new RowMapper<TimeOverlap>() {
					@Override
					public TimeOverlap mapRow(ResultSet rs, int rowNum)
						throws SQLException{
						TimeOverlap cls = new TimeOverlap(rs.getString("starttime"),
	...
	public String starttime(String km_code) {
		return jdbcTemplate.queryForObject("select left(km_time,1) from bombombom.subject where km_code = ?",String.class, km_code);
	}
	
	public String endtime(String km_code) {
		return jdbcTemplate.queryForObject("select right(km_time,1) from bombombom.subject where km_code = ?",String.class, km_code);
	}
